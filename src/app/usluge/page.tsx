  ...
  const handleSearch = (upit: string, cena: number) => {
    const rezultati = sveUsluge.filter((u: any) => {
      const poklapaTekst =
        ...
        u.preduzece_naziv.toLowerCase().includes(upit.toLowerCase());
    ...
    setPoruka(
      rezultati.length ? "Usluge su pronađene." : "Usluga nije pronađena."
    );
  };
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtriraneUsluge.map((u: any) => (
              <div
                key={u.idusluga}
                className="bg-white rounded-xl overflow-hidden shadow-md border border-gray-100 hover:shadow-lg transition"
              >
